...
const ERROR_CODES = [
  { code: 'RATE_LIMIT_EXCEEDED',   status: 429, description: 'Too many requests. See Retry-After header.' },
  { code: 'RUN_NOT_FOUND',         status: 404, description: 'The specified run_id does not exist or has expired.' },
  { code: 'TICKER_NOT_RESOLVABLE', status: 422, description: 'The ticker could not be resolved to a known company.' },
  { code: 'INVALID_TICKER_FORMAT', status: 422, description: 'The ticker contains invalid characters or is too long.' },
  { code: 'PIPELINE_TIMEOUT',      status: 504, description: 'The pipeline exceeded the maximum allowed duration.' },
  { code: 'INTERNAL_ERROR',        status: 500, description: 'An unexpected server error occurred.' },
];

// ─── Rate limit table ─────────────────────────────────────────────────────────

const RATE_LIMITS = [
  { endpoint: 'POST /api/v1/analyze',          limit: '10 requests / 60 seconds per IP' },
  { endpoint: 'GET /api/v1/analyze/stream/:id', limit: 'No rate limit (read-only)' },
...
export default function DocsPage() {
  return (
    <main id="main-content" className="max-w-3xl mx-auto px-4 py-10">
      ...
      {/* ── Endpoints ──────────────────────────────────────────────────────── */}
      <h2 className="text-lg font-semibold text-neutral-800 mb-4">Endpoints</h2>
      ...
      <section className="mb-8">
        <h2 className="text-lg font-semibold text-neutral-800 mb-3">Rate Limits</h2>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {RATE_LIMITS.map(({ endpoint, limit }) => (
                <tr key={endpoint} className="border-b border-neutral-100 last:border-0">
                  <td className="px-4 py-2 font-mono text-xs text-neutral-700">
                    {endpoint}
                  </td>
                  <td className="px-4 py-2 text-xs text-neutral-600">{limit}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <p className="mt-2 text-xs text-neutral-500">
          ...
          <code className="px-1 py-0.5 bg-neutral-100 rounded font-mono">Retry-After</code>{' '}
      ...
      {/* ── Error codes ────────────────────────────────────────────────────── */}
      <section className="mb-8">
        <h2 className="text-lg font-semibold text-neutral-800 mb-3">Error Codes</h2>
  ...
  "retry_after": 60
            ...
            <tbody>
              {ERROR_CODES.map(({ code, status, description }) => (
                <tr key={code} className="border-b border-neutral-100 last:border-0">
                  ...
                  <td className="px-4 py-2 text-xs text-neutral-600">{status}</td>
                  <td className="px-4 py-2 text-xs text-neutral-600">{description}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <footer className="mt-10 pt-6 border-t border-neutral-200">
        <p className="text-xs text-neutral-400">
          StockLens AI is for informational purposes only and does not constitute
