...
const STEP_DESCRIPTIONS: Record<string, string> = {
  ...
  market_data_collection:
    "Fetching current price, volume, and financial ratios",
...
function StepStatusIcon({ status }: { status: StepStatus }) {
  switch (status) {
    ...
    default:
      return (
        <Loader2
          className="w-3.5 h-3.5 text-muted-foreground animate-spin shrink-0 mt-0.5"
          aria-hidden="true"
        />
      );
  }
...
export function ReasoningViewer() {
  ...
  const { activeModel, openAiKeyStatus } = useSettingsStore();

  return (
    <section
      aria-label="Analysis reasoning steps"
      aria-live="polite"
      className="mb-4 rounded-xl border border-border bg-muted/30 px-4 py-3"
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          {openAiKeyStatus === "set" && activeModel && (
        ...
        <button
          ...
          className="text-xs text-muted-foreground hover:text-foreground min-h-[44px] px-2 focus:outline-none focus:ring-2 focus:ring-brand-500 rounded-lg transition-colors"
        >
      ...
      <ol
        ...
        hidden={!reasoningViewerExpanded}
      >
